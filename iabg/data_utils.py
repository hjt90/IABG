...
from iabg.utils import read_csv, write_csv
...
def gather_movielens_info(vocab=None, ngrams=1, min_freq=1):
    csv_path = os.path.join("./data", 'movielens', 'click.csv')
    if vocab is None:
    ...
    with tqdm(unit_scale=0, unit='lines') as t:
        for tokens in iterator:
            ...
            t.update(1)
    csv_path = os.path.join("./data", 'movielens', 'buy.csv')
    iterator = _csv_iterator(csv_path, ngrams)
    num_buys = 0
    with tqdm(unit_scale=0, unit='lines') as t:
        for tokens in iterator:
...
def gather_yoochoose_info(vocab=None, ngrams=1, min_freq=1):
    csv_path = os.path.join("./data", 'yoochoose', 'click.csv')
    if vocab is None:
        vocab = _build_vocab(_csv_iterator(csv_path, ngrams), min_freq=min_freq)
    iterator = _csv_iterator(csv_path, ngrams)
    num_users, num_clicks = 0, 0
    with tqdm(unit_scale=0, unit='lines') as t:
        for tokens in iterator:
            ...
            num_clicks += len(tokens)
            t.update(1)
    csv_path = os.path.join("./data", 'yoochoose', 'buy.csv')
    iterator = _csv_iterator(csv_path, ngrams)
    num_buys = 0
    with tqdm(unit_scale=0, unit='lines') as t:
        for tokens in iterator:
