...
class Collector(object):
    ...
    def collect(
        ...
        episode_r, episode_p, episode_pp=[], [], []
        ...
        while True:
            ...
            if np.any(done):
                ...
                success=np.zeros_like(episode_rews[-1])
                ndcg=np.zeros_like(episode_rews[-1])
                recall = np.zeros_like(episode_rews[-1])
                precision = np.zeros_like(episode_rews[-1])
                precision_plus = np.zeros_like(episode_rews[-1])
                for i,v in enumerate(info[env_ind_local]):
                    # if v > 0.5:   #todo：根据env的reward来判断具体的success与否
                    #     success[i]=1.0
                    recall[i] = v['recall']
                    precision[i] = v['precision']
                    precision_plus[i] = v['precision_plus']
                    # print(v)
                episode_r.append(recall)
                episode_p.append(precision)
                episode_pp.append(precision_plus)
                # episode_sr.append(success)
                # episode_ndcg.append(ndcg)
        ...
        if episode_count > 0:
            rews, lens, idxs, recall, precision, precision_plus = list(map(np.concatenate,
                    [episode_rews, episode_lens, episode_start_indices,episode_r,episode_p,episode_pp]))
            rew_mean, rew_std = rews.mean(), rews.std()
            len_mean, len_std = lens.mean(), lens.std()
            recall = recall.mean()
            precision = precision.mean()
            precision_plus = precision_plus.mean()
        else:
            rews, lens, idxs = np.array([]), np.array([], int), np.array([], int)
            recall, precision, precision_plus = 0.0, 0.0, 0.0
